...
func GetReleaseResources(
	...
	cfg := cli.GetConfig()
	resources, err := cfg.KubeClient.Build(bytes.NewBufferString(rel.Manifest), true)
	if err != nil {
		return nil, err
	}
	ret := make(map[string][]interface{})
	resources.Visit(func(info *resource.Info, err error) error {
		kind := strings.ToLower(info.Object.GetObjectKind().GroupVersionKind().Kind)
		obj, err := convertRuntimeObj(indexer, cluster, info.Object, rel.Namespace)
		if err != nil {
			return err
		}
		if list, ok := ret[kind]; ok {
