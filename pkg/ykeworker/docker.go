...
type NodeConfig struct {
	...
	Processes   map[string]yketypes.Process `json:"processes"`
	Files       []ytypes.File               `json:"files"`
}

func runProcess(ctx context.Context, name string, p yketypes.Process, start bool) error {
...
func changed(ctx context.Context, c *client.Client, p yketypes.Process, container types.Container) (bool, error) {
	inspect, err := c.ContainerInspect(ctx, container.ID)
	if err != nil {
		return false, err
	}

	newProcess := yketypes.Process{
