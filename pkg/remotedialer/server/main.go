...
func Client(server *remotedialer.Server, rw http.ResponseWriter, req *http.Request) {
	...
	log.Infof("[%03d] REQ t=%s %s", id, timeout, url)

	resp, err := client.Get(url)
	if err != nil {
		log.Errorf("[%03d] REQ ERR t=%s %s: %v", id, timeout, url, err)
		onError(rw, req, 500, err)
		return
	}
	defer resp.Body.Close()

	log.Infof("[%03d] REQ OK t=%s %s", id, timeout, url)
	rw.WriteHeader(resp.StatusCode)
	io.Copy(rw, resp.Body)
	log.Infof("[%03d] REQ DONE t=%s %s", id, timeout, url)
}
